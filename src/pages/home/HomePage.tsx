import type { HomeLoaderResult } from "./homeLoade";
import { useLoaderData, Link } from "react-router-dom";

export default function HomePage() {
  const { featuredPackages } = useLoaderData() as HomeLoaderResult;

  const renderedPackages = featuredPackages.map((feature) => {
    return (
      <div
        key={feature.name}
      ...
      >
        <div className="flex flex-col gap-1 border-bottom border-gray-400">
          <div className="font-bold text-center">{feature.name}</div>
          <div className="text-sm text-gray-500">{feature.description}</div>
          <div className="text-sm text-gray-500">
            {feature.maintainers.length} Maintainers
          </div>
        </div>
        <Link
          to={`/packages/${feature.name}`}
